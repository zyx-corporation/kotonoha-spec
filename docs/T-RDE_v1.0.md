...
> **正本（canonical）:** 本ファイル [`kotonoha-spec/docs/T-RDE_v1.0.md`](https://github.com/zyx-corporation/kotonoha-spec/blob/main/docs/T-RDE_v1.0.md) が T-RDE v1.0 理論ガイドの唯一の公開正本である。監査セッション用の **実行プロンプト** 正本は [`t_rde_実行プロンプト_v_1_論文準拠.md`](t_rde_実行プロンプト_v_1_論文準拠.md)（**v1.1a** — §9 最小版 / §5 詳細版 / §10 短縮テンプレート）。
...
T-RDE v1.0の理論的基盤は、公開済み・参照可能なRTI（Resonance Theory of Intelligence）とΔM価値生成論に置く。RDE（Resonant Deviation Evaluator）軸によるΔM五成分の細分化は、RDE論文の公開・参照可能化を待ってv1.1以降で扱う（§14参照）。
...
| RDE軸の保留 | RDE論文が投稿済み・未公開のため、v1.0では中間層として導入せず、RTI+ΔMを主基盤とする。v1.1以降の拡張候補 |
...
**運用:** コピー＆ペーストで使う監査用プロンプトは、正本 [`t_rde_実行プロンプト_v_1_論文準拠.md`](t_rde_実行プロンプト_v_1_論文準拠.md)（**v1.1a** — §9 最小 / §5 詳細 / §10 短縮テンプレート）を使う。以下は短縮版である。
...
**RDE軸との接続はv1.1以降の拡張課題**：T-RDE v1.0では、ΔM五成分を主要な観測レンズとして採用し、RDE（Resonant Deviation Evaluator）軸を正式な中間層としては導入しない。これは、RDE論文の最新版が投稿済みである一方、jXiv等でまだ公開・参照可能な状態にないためである。T-RDE v1.0が未公開文献に強く依存すると、実践ガイドとしての再利用性・検証可能性が下がる。RDE軸は、Claim Strength、Intent、Context Affinity、Value Structure、Responsibilityなどを通じて、ΔM五成分では粗くなりがちな監査質問を細分化する有力な中間観測層である。たとえば、ΔRに直接ラベルすると「関係性の変化」としか記述できない意味変化が、RDEのValue & Power dynamics軸を経由することで「価値序列の可視化がユーザーの判断にどう影響するか」という問いに展開できる。T-RDE v1.0ではRTIとΔMを主基盤とし、RDE軸との対応表、L2/L3監査での導入方法、UIBとの接続は、RDE論文公開後のv1.1以降で扱う。
